...
function printHelp() {
  ...
  if [ "$USAGE" == "prereq" ]; then
    ...
    println "    -ccep <policy>  - (Optional) Chaincode endorsement policy using signature policy syntax. The default policy requires an endorsement from Orgregistrocivil and Orgregistropolicial"
