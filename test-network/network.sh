...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating Orgregistrocivil Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-orgregistrocivil.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating Orgregistropolicial Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-orgregistropolicial.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    peer_cert peer peer0.orgregistrocivil.example.com orgregistrocivil
    peer_cert admin Admin@orgregistrocivil.example.com orgregistrocivil

    infoln "Creating Orgregistropolicial Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.orgregistropolicial.example.com orgregistropolicial
    peer_cert admin Admin@orgregistropolicial.example.com orgregistropolicial
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/orgregistrocivil/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
    ...
    while [[ $rc -ne 0 && $COUNTER -lt $MAX_RETRY ]]; do
      sleep 1
      set -x
      fabric-ca-client getcainfo -u https://admin:adminpw@localhost:7054 --caname ca-orgregistrocivil --tls.certfiles "${PWD}/organizations/fabric-ca/orgregistrocivil/ca-cert.pem"
      res=$?
    { set +x; } 2>/dev/null
    rc=$res  # Update rc
    COUNTER=$((COUNTER + 1))
    ...
    createOrgregistrocivil

    infoln "Creating Orgregistropolicial Identities"

    createOrgregistropolicial
  ...
  infoln "Generating CCP files for Orgregistrocivil and Orgregistropolicial"
...
# call the script to create the channel, join the peers of orgregistrocivil and orgregistropolicial,
...
function networkDown() {
  ...
  # stop orgregistropropiedad containers also in addition to orgregistrocivil and orgregistropolicial, in case we were running sample to add orgregistropropiedad
  COMPOSE_ORG3_BASE_FILES="-f addOrgregistropropiedad/compose/${COMPOSE_FILE_ORG3_BASE} -f addOrgregistropropiedad/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_ORG3_BASE}"
  COMPOSE_ORG3_COUCH_FILES="-f addOrgregistropropiedad/compose/${COMPOSE_FILE_ORG3_COUCH} -f addOrgregistropropiedad/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_ORG3_COUCH}"
  COMPOSE_ORG3_CA_FILES="-f addOrgregistropropiedad/compose/${COMPOSE_FILE_ORG3_CA} -f addOrgregistropropiedad/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_ORG3_CA}"
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_peer0.orgregistrocivil.example.com docker_peer0.orgregistropolicial.example.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/orgregistrocivil/msp organizations/fabric-ca/orgregistrocivil/tls-cert.pem organizations/fabric-ca/orgregistrocivil/ca-cert.pem organizations/fabric-ca/orgregistrocivil/IssuerPublicKey organizations/fabric-ca/orgregistrocivil/IssuerRevocationPublicKey organizations/fabric-ca/orgregistrocivil/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/orgregistropolicial/msp organizations/fabric-ca/orgregistropolicial/tls-cert.pem organizations/fabric-ca/orgregistropolicial/ca-cert.pem organizations/fabric-ca/orgregistropolicial/IssuerPublicKey organizations/fabric-ca/orgregistropolicial/IssuerRevocationPublicKey organizations/fabric-ca/orgregistropolicial/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/ordererOrg/msp organizations/fabric-ca/ordererOrg/tls-cert.pem organizations/fabric-ca/ordererOrg/ca-cert.pem organizations/fabric-ca/ordererOrg/IssuerPublicKey organizations/fabric-ca/ordererOrg/IssuerRevocationPublicKey organizations/fabric-ca/ordererOrg/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf addOrgregistropropiedad/fabric-ca/orgregistropropiedad/msp addOrgregistropropiedad/fabric-ca/orgregistropropiedad/tls-cert.pem addOrgregistropropiedad/fabric-ca/orgregistropropiedad/ca-cert.pem addOrgregistropropiedad/fabric-ca/orgregistropropiedad/IssuerPublicKey addOrgregistropropiedad/fabric-ca/orgregistropropiedad/IssuerRevocationPublicKey addOrgregistropropiedad/fabric-ca/orgregistropropiedad/fabric-ca-server.db'
...
# use this as the default docker-compose yaml definition for orgregistropropiedad
COMPOSE_FILE_ORG3_BASE=compose-orgregistropropiedad.yaml
# use this as the docker compose couch file for orgregistropropiedad
COMPOSE_FILE_ORG3_COUCH=compose-couch-orgregistropropiedad.yaml
# certificate authorities compose file
COMPOSE_FILE_ORG3_CA=compose-ca-orgregistropropiedad.yaml
#
